...
class Email(object):
    def __init__(self, key, msg, original_date=None):
        ...
        if msg:
            ...
            self.fr = u"; ".join([decode_email_address(f, charset) for f in fr_addr])
            self.to = u"; ".join([decode_email_address(t, charset) for t in to_addr])
            self.cc = u"; ".join([decode_email_address(t, charset) for t in cc_addr])
        self.errors = []
        self.destinations = []
    ...
    def place_attachments(self, eml, destination_dir, attachment_is_local=False):
        for attachment in eml.attachments:
            ...
            if attachment_is_local:
                ...
                data = StringIO(attachment.get_payload(decode=True))
